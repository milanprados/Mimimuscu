...
const compiled = compileProgramBlocks([{
  type: "exercise", exercise_id: "pushups", sets: 999,
  rest_after_sec: 99999, target_scale: 99
}]);
...
const exported = sessionToFile({
  name: "Test", blocks: [{type:"exercise", exercise_id:"squat"}],
  exercises: [{id:"pushups", phase:"Personnalisé", restAfter:30, targetOverride:12}]
});
...
assert.throws(() => validateImportedSession({
  app:"mimi-muscu", type:"workout-session", name:"cassée",
...
store.set("mimiMuscuV31", JSON.stringify({
  version: 31,
  program: {index: "Infinity"},
  sessions: "12", attempts: "13", xp: "900", streak: "4",
  targets: [], bests: [],
  customSessions: [{id:"x", name:"x", exercises:[{id:"__missing__"},{id:"pushups",restAfter:9999}]}],
...
const exercise = {id:"pushups",name:"Pompes",mode:"reps",base:5,min:1,max:50,step:1};
const partialState = {
  program:{level:"adaptive",index:0}, sessionDraft:null,
  targets:{pushups:5}, bests:{}, sessions:0, attempts:0, xp:0,
  streak:0,lastDay:"",history:[]
};
const partialPlan = {
  items:Array.from({length:4},()=>({kind:"exercise",id:"pushups",phase:"Bloc principal",track:true,adaptive:true,targetOverride:5})),
  meta:{key:"PARTIAL",name:"Partial",type:"program"}
};
let finishedPayload;
const engine = new WorkoutEngine({
  state:partialState, exercises:{pushups:exercise},
  buildProgramPlan:()=>partialPlan, buildSessionPlan:()=>partialPlan,
  persist:()=>{}, now:()=>new Date(2026,7,11,12,0,0),
  hooks:{countdown:({done})=>done(), exercise:()=>{}, finished:p=>{finishedPayload=p;}}
});
engine.start();
...
console.log("OK — stabilité V31.4 : entrées bornées, état réparé, séances partielles, export, finish idempotent.");
