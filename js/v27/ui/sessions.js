...
export function initSessions({
  ...
  function savePlannerDraft(){
    ...
    state.sessionDraft={templateKey:tpl.key,level:"adaptive",exercises:JSON.parse(JSON.stringify(plannerItems))};
    save(state);
  }
  ...
  on("#startPlannedSession","click",()=>{
    savePlannerDraft();
    const tpl=workoutTemplate(state.program);
    const session={id:`program-${tpl.key}`,name:tpl.name,description:tpl.focus,level:"adaptive",week:tpl.week,day:tpl.day,exercises:JSON.parse(JSON.stringify(plannerItems))};
