...
export function initDashboard({
 ...
 workoutTemplate,compressSessionForDuration,launchSession,openPlanner,refresh
  ...
  function renderMilestones(){
    ...
    $("#milestonePreview").innerHTML=safeMilestones.slice(0,3).map(m=>{
    ...
    $("#milestoneList").innerHTML=safeMilestones.map(m=>{
      const p=milestoneProgress(m),best=state.bests[m.exerciseId]||0;
  ...
  function render(){
    ...
    $("#rotationText").textContent=(Array.isArray(PROGRAMS[state.program.level])?PROGRAMS[state.program.level]:(Array.isArray(PROGRAMS.beginner)?PROGRAMS.beginner:[])).map(x=>x.key).join(" → ");
    renderMilestones();
  }
