...
const KEY="mimiMuscuV30";
const OLD_KEYS=["mimiMuscuV29","mimiMuscuV26","mimiMuscuV22","mimiMuscuV20","coachV20","coachV19","coachV18"];
...
export function load(exercises){
  ...
  if(state.program.level!=="adaptive"){state.program={level:"adaptive",index:0};state.sessionDraft=null;}
  ...
  state.profileMeta={...DEFAULT.profileMeta,...(state.profileMeta||{})};
  state.progressBaseline={...DEFAULT.progressBaseline,...(state.progressBaseline||{}),values:{...(state.progressBaseline?.values||{})}};
  state.goals=Array.isArray(state.goals)?state.goals:[];
  state.calendarPrefs={...DEFAULT.calendarPrefs,...(state.calendarPrefs||{})};
  state.calendarPrefs.restDay=Math.max(0,Math.min(6,Number(state.calendarPrefs.restDay)||0));
