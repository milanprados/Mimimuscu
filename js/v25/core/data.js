...
export function workoutTemplate(program={}){
  const level = PROGRAMS[program?.level] ? program.level : "beginner";
  const list = Array.isArray(PROGRAMS[level]) ? PROGRAMS[level] : [];
  if(!list.length) throw new Error(`Aucun programme disponible pour ${level}`);
  const index = Math.abs(Number(program?.index)||0) % list.length;
  const template = list[index] || list[0];
  const compiled = compileBlocksToSessionExercises(Array.isArray(template.blocks)?template.blocks:[]);
  return {
    ...template,
    blocks:Array.isArray(template.blocks)?template.blocks:[],
    ids:compiled.map(item=>item.id).filter(Boolean)
  };
}
  ...
  for(const block of (Array.isArray(blocks)?blocks:[])){
    if(block.type==="exercise"){
      ...
      for(let i=0;i<sets;i++) out.push({
        ...
        tempo:block.tempo||null,note:block.note||"",test:!!block.test
      });
...
export function buildPlan(program){
  ...
  const exercises=compileBlocksToSessionExercises(tpl.blocks);
  const warm=[
    {id:"squat",phase:"Échauffement",warm:true,targetOverride:30,modeOverride:"time",restAfter:25},
    {id:"reverse_lunge",phase:"Échauffement",warm:true,targetOverride:6,restAfter:25}
  ];
  const main=exercises.map((x,i)=>({...x,phase:i<Math.ceil(exercises.length/2)?"Bloc 1":"Bloc 2"}));
  return interleave([...warm,...main],tpl);
}
