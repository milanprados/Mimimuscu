...
export const RECORD_EXERCISE_IDS = [
  "pushups", "plank", "squat", "reverse_lunge",
  "pike", "reverse_crunch", "superman_pull", "single_leg_glute_bridge"
];

export const TARGET_EXERCISE_IDS = [
  "pushups", "pike", "squat", "reverse_lunge",
  "plank", "dead_bug", "superman_pull", "reverse_snow_angel"
];

export const MUSCLE_GROUPS = [
  "Pecs", "Épaules", "Triceps", "Dos", "Jambes", "Fessiers", "Core"
];
...
function firstActual(state, exerciseId) {
  for (const record of [...countedHistory(state)].reverse()) {
    const set = (record.sets || []).find(item =>
...
function currentPerformance(state, exercises, exerciseId) {
  ...
  return Number(state.targets[exerciseId]) || Number(exercises[exerciseId]?.base) || 1;
}
...
export function ensureProgressBaseline(state, exercises) {
  ...
  const anchorIds = [...new Set(
    Object.values(PROGRESSION_AXES).flatMap(axis => axis.ids)
  )];
  ...
  if (!state.progressBaseline.createdAt) {
    ...
    state.progressBaseline.createdAt = firstRecord?.date || new Date().toISOString();
    changed = true;
  }
...
function buildIndices(state, exercises, now) {
  ...
  return {
    ...axes,
    physical,
    regularity: Math.round(clamp(activeDays / PROGRAM.cycleLength * 100, 0, 100))
  };
}
...
function buildTotals(state) {
  ...
  return {
    ...
    averageScore: scored.length
      ? Math.round(scored.reduce((sum, record) => sum + Number(record.score), 0) / scored.length)
      : 0
  };
}
...
function buildBodyData(state) {
  ...
  return {
    ...
    weightDelta: currentWeight && firstWeight
      ...
      : null,
    waistDelta: currentWaist && firstWaist
      ? roundOne(currentWaist - firstWaist)
      : null
  };
}
