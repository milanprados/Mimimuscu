...
export function createSessionPlanner({
  ...
  const customById = (id) =>
    state.customSessions.find((session) => session.id === id);
  const plannerTarget = (id) => {
    const ex = EXERCISES[id],
      t = state.targets[id] ?? ex.base;
  ...
  function renderCustomSessions() {
    ...
    root.innerHTML = (
      Array.isArray(state.customSessions) ? state.customSessions : []
    )
      .map(
        (session) => `
    ...
    $$("[data-custom-start]").forEach((b) =>
      b.addEventListener("click", () => {
        const s = customById(b.dataset.customStart);
        if (s) launchSession(s, false);
      }),
    );
    $$("[data-custom-edit]").forEach((b) =>
      b.addEventListener("click", () => openCustomEditor(b.dataset.customEdit)),
    );
    $$("[data-custom-export]").forEach((b) =>
      b.addEventListener("click", () => {
        const s = customById(b.dataset.customExport);
        if (s) downloadSessionFile(s);
      }),
    );
  }
  ...
  function openCustomEditor(id = null) {
    ...
    $("#customEditorTitle").textContent = existing
      ? "Modifier la séance"
      : "Nouvelle séance";
    ...
    renderCustomEditor();
    openLayer("#customEditor");
  }

  function closeCustomEditor() {
    closeLayer("#customEditor");
  }

  function renderCustomEditor() {
    $("#customEditorList").innerHTML = (
      Array.isArray(customWorking.exercises) ? customWorking.exercises : []
    )
      .map((item, index) => {
        ...
        ${
          index < customWorking.exercises.length - 1
            ? `<div class="custom-rest">Repos après :
    ...
    $$("[data-custom-up]").forEach((b) =>
      b.addEventListener("click", () => {
        const i = +b.dataset.customUp;
        [customWorking.exercises[i - 1], customWorking.exercises[i]] = [
          customWorking.exercises[i],
          customWorking.exercises[i - 1],
        ];
        renderCustomEditor();
      }),
    );
    $$("[data-custom-down]").forEach((b) =>
      b.addEventListener("click", () => {
        const i = +b.dataset.customDown;
        [customWorking.exercises[i + 1], customWorking.exercises[i]] = [
          customWorking.exercises[i],
          customWorking.exercises[i + 1],
        ];
    ...
    $$("[data-custom-delete]").forEach((b) =>
      b.addEventListener("click", () => {
        customWorking.exercises.splice(+b.dataset.customDelete, 1);
        renderCustomEditor();
      }),
    );
    $$("[data-custom-rest]").forEach((input) =>
      input.addEventListener("change", () => {
        const i = +input.dataset.customRest;
        customWorking.exercises[i].restAfter = Math.max(
          0,
          Math.min(180, +input.value || 0),
        );
      }),
    );
    $$("[data-custom-replace]").forEach((b) =>
      b.addEventListener("click", () => {
        pickerContext = "custom-replace";
        replaceIndex = +b.dataset.customReplace;
        openPicker();
      }),
    );
  }
  ...
  function saveCustomEditor() {
    customWorking.name =
      $("#customName").value.trim() || "Séance personnalisée";
    ...
    const index = state.customSessions.findIndex(
      (s) => s.id === customWorking.id,
    );
  ...
  const targetForItem = (item) => {
    ...
    if (explicit != null)
      return `${Math.round(explicit)}${(item.modeOverride || ex.mode) === "time" ? " sec" : ex.perSide ? " / côté" : " reps"}`;
  ...
  const defaultPlannerItems = () =>
    JSON.parse(JSON.stringify(programSession(state.program).exercises || []));
  function loadPlanner() {
    const tpl = workoutTemplate(state.program),
      draft = state.sessionDraft;
    const valid =
      draft && draft.templateKey === tpl.key && Array.isArray(draft.exercises);
    plannerItems = valid
      ? JSON.parse(JSON.stringify(draft.exercises))
      : defaultPlannerItems();
  }
  function savePlannerDraft() {
    ...
    state.sessionDraft = {
      templateKey: tpl.key,
      exercises: JSON.parse(JSON.stringify(plannerItems)),
    };
    save(state);
  }
  function renderPlanner() {
    const tpl = workoutTemplate(state.program);
    $("#plannerLabel").textContent =
      `Semaine ${tpl.week} • Jour ${tpl.day}/6 • ~20 min`;
    $("#plannerName").textContent = tpl.name;
    $("#plannerMeta").textContent =
      `Échauffement → full body → retour au calme • ${tpl.focus}`;
    $("#plannerList").innerHTML =
      (Array.isArray(plannerItems) ? plannerItems : [])
        .map((item, index) => {
          const ex = EXERCISES[item.id];
          if (!ex) return "";
          const rest =
            index < plannerItems.length - 1 ? (item.restAfter ?? 0) : 0;
        ...
        .join("") +
      `<button class="add-exercise" id="addExercise">＋ Ajouter un exercice</button>`;

    $$("[data-up]").forEach((b) =>
      b.addEventListener("click", () => {
        const i = +b.dataset.up;
        [plannerItems[i - 1], plannerItems[i]] = [
          plannerItems[i],
          plannerItems[i - 1],
        ];
        savePlannerDraft();
    ...
    $$("[data-delete]").forEach((b) =>
      b.addEventListener("click", () => {
        if (plannerItems.length <= 1) return;
        plannerItems.splice(+b.dataset.delete, 1);
        savePlannerDraft();
        renderPlanner();
      }),
    );
    $$("[data-replace]").forEach((b) =>
      b.addEventListener("click", () => {
        ...
        pickerMode = "replace";
        replaceIndex = +b.dataset.replace;
        openPicker();
      }),
    );
    on("#addExercise", "click", () => {
      pickerContext = "planner";
      pickerMode = "add";
      replaceIndex = null;
      openPicker();
  ...
  function openPicker() {
    $("#pickerSearch").value = "";
    renderPicker();
    openLayer("#exercisePicker");
  }
  function closePicker() {
    closeLayer("#exercisePicker", { keepLocked: true });
  }
  function renderPicker() {
    ...
    const entries = Object.entries(EXERCISES)
      .filter(
        ([, e]) =>
          !q ||
          `${e.name} ${e.primary} ${e.secondary}`.toLowerCase().includes(q),
      )
      .sort((a, b) => a[1].name.localeCompare(b[1].name, "fr"));
    $("#pickerList").innerHTML = entries
      .map(
        ([id, e]) => `<button class="picker-card" data-pick="${id}">
      <img src="${e.thumb || e.images[0]}" alt=""><div><strong>${e.name}</strong><small>${e.primary}</small></div><span>＋</span></button>`,
      )
      .join("");
    $$("[data-pick]").forEach((b) =>
      b.addEventListener("click", () => {
        ...
        if (pickerContext === "custom-replace" && replaceIndex !== null) {
          const cur = customWorking.exercises[replaceIndex] || {
            restAfter: 25,
          };
          customWorking.exercises[replaceIndex] = { ...cur, id };
          closePicker();
          renderCustomEditor();
          pickerContext = "planner";
          return;
  ...
  on("#deleteCustomSession", "click", () => {
    ...
    state.customSessions = state.customSessions.filter(
      (s) => s.id !== customEditingId,
    );
    save(state);
    closeCustomEditor();
    renderCustomSessions();
