...
export function createCalendarView({state,save,PROGRAMS,programSession,launchSession}) {
  let viewDate=startOfDay(new Date());
  let selectedKey=localDayKey(viewDate);

  const today=()=>startOfDay(new Date());
  const todayKey=()=>localDayKey(today());
  const programs=()=>Array.isArray(PROGRAMS.adaptive)?PROGRAMS.adaptive:[];
  const restDay=()=>Number(state.calendarPrefs?.restDay)||0;
  const programHistoryFor=key=>state.history.filter(h=>h.day===key&&isProgramHistoryRecord(h)&&(h.programCompleted===true||h.counted));
  ...
  function scheduleMap(days=400){ return new Map(schedule(days).map(x=>[x.key,x])) }

  function dateStatus(date,smap=scheduleMap()){
    const key=localDayKey(date),actual=countedFor(key),programActual=programHistoryFor(key);
    if(programActual.length)return{type:"done",key,actual:programActual[0],all:historiesFor(key)};
    if(key<todayKey()&&actual.length)return{type:"done",key,actual:actual[0],all:historiesFor(key)};
    if(key<todayKey()){
  ...
  function renderHome(){
    const now=today(),smap=scheduleMap(60),status=dateStatus(now,smap);
    $("#calendarTodayDate").textContent=formatLongDate(now);
    const title=$("#calendarTodayTitle"),sub=$("#calendarTodaySub"),badge=$("#calendarTodayBadge");
    if(status.type==="done"){
      ...
      badge.textContent="FAIT";badge.className="calendar-status done";
    }else if(status.type==="rest"){
      const next=schedule(20).find(x=>x.type==="planned");
      title.textContent="Repos prévu";
      sub.textContent=next?`Prochaine séance : ${next.template?.name||"Séance"} · ${formatShortDate(next.date)}`:"Récupération";
      badge.textContent="REPOS";badge.className="calendar-status rest";
      ...
      badge.textContent=status.planned.template?.intensity==="léger"?"LÉGER":"PRÉVU";
      badge.className="calendar-status planned";
    }else{
      title.textContent="Journée libre";sub.textContent="Aucune séance planifiée.";badge.textContent="LIBRE";badge.className="calendar-status";
    }

    const monday=mondayOfWeek(now);
    $("#homeWeekStrip").innerHTML=Array.from({length:7},(_,i)=>{
      const d=addDays(monday,i),st=dateStatus(d,smap),isToday=localDayKey(d)===todayKey();
    ...
    const nextRows=schedule(14).filter(x=>x.key>todayKey()).slice(0,3);
    $("#homeUpcoming").innerHTML=nextRows.map(x=>{
      if(x.type==="rest")return`<div class="home-next-row rest"><span>${formatShortDate(x.date)}</span><strong>Repos</strong><em>récupération</em></div>`;
  ...
  function renderMonth(){
    const year=viewDate.getFullYear(),month=viewDate.getMonth(),smap=scheduleMap(420);
    $("#calendarMonthLabel").textContent=`${MONTH_NAMES[month]} ${year}`;
    const cells=monthCells(year,month);
    $("#calendarGrid").innerHTML=cells.map(date=>{
      const key=localDayKey(date),st=dateStatus(date,smap);
      const outside=date.getMonth()!==month,todayClass=key===todayKey()?"today":"",selected=key===selectedKey?"selected":"";
      const marker=st.type==="done"?"✓":st.type==="planned"?"•":st.type==="rest"?"—":"";
      const meta=st.type==="planned"?`<small>S${st.planned.week}J${st.planned.day}</small>`:st.type==="done"?`<small>fait</small>`:st.type==="rest"?`<small>repos</small>`:"";
    ...
    $$("[data-date]").forEach(b=>b.addEventListener("click",()=>{
      ...
      const d=parseLocalDayKey(selectedKey);viewDate=new Date(d.getFullYear(),d.getMonth(),1);
      renderMonth();renderSelected();
    }));
    renderSelected();
    renderUpcoming();
  }
  ...
  function renderSelected(){
    const date=parseLocalDayKey(selectedKey),st=dateStatus(date),box=$("#calendarSelected");
    $("#calendarSelectedDate").textContent=formatLongDate(date);
    if(st.type==="done"){
      const all=historiesFor(selectedKey),primary=st.actual;
      ...
      on("#calendarStartToday","click",()=>{closeLayer("#calendarModal");launchSession(programSession(state.program),false)},{required:false});
  ...
  function renderUpcoming(){
    const rows=schedule(30).filter(x=>x.key>=todayKey()).slice(0,10);
    $("#calendarUpcomingList").innerHTML=rows.map(x=>{
      const actual=countedFor(x.key)[0];
      if(actual)return`<div class="calendar-agenda done"><time>${formatShortDate(x.date)}</time><div><strong>${actual.sessionName||"Séance"}</strong><small>${actual.score}/100 · terminé</small></div><span>✓</span></div>`;
      if(x.type==="rest")return`<div class="calendar-agenda rest"><time>${formatShortDate(x.date)}</time><div><strong>Repos</strong><small>récupération</small></div><span>—</span></div>`;
      if(x.type==="planned")return`<div class="calendar-agenda planned"><time>${formatShortDate(x.date)}</time><div><strong>${x.template?.name||"Séance"}</strong><small>S${x.week} · J${x.day}/6 · ${x.template?.intensity||""}</small></div><span>→</span></div>`;
      return"";
    }).join("");
  }

  function openCalendar(){
    viewDate=new Date(today().getFullYear(),today().getMonth(),1);selectedKey=todayKey();
    $("#restDaySelect").value=String(restDay());
    renderMonth();openLayer("#calendarModal");
  }
  ...
  on("#calendarPrev","click",()=>{viewDate=new Date(viewDate.getFullYear(),viewDate.getMonth()-1,1);renderMonth()});
  on("#calendarNext","click",()=>{viewDate=new Date(viewDate.getFullYear(),viewDate.getMonth()+1,1);renderMonth()});
  on("#calendarTodayBtn","click",()=>{viewDate=new Date(today().getFullYear(),today().getMonth(),1);selectedKey=todayKey();renderMonth()});
  on("#restDaySelect","change",()=>{
    state.calendarPrefs.restDay=Number($("#restDaySelect").value);save(state);renderHome();renderMonth();
  });
