...
export function initDictionary({EXERCISES}) {
  ...
  function openGuide(id){
    ...
    $("#modalBody").innerHTML=`
      ...
      <div class="guide-section"><h3>Exécution</h3><ol>${e.tips.map(x=>`<li>${x}</li>`).join("")}</ol></div>
      <div class="guide-section"><h3>Respiration</h3><p>${e.breathing}</p></div>
      <div class="guide-section"><h3>Erreurs fréquentes</h3><ul>${e.mistakes.map(x=>`<li>${x}</li>`).join("")}</ul></div>
