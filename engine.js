...
export class Engine{
  ...
  resetRuntime(){this.plan=[];this.i=-1;this.timer=null;this.remaining=0;this.running=false;this.log=[];this.startedAt=null;this.phaseAt=null;this.reps=0;this.effort="good";this.redo=false}
  ex(){const x=this.plan[this.i];return x?.kind==="exercise"?EXERCISES[x.id]:null}
  target(id,item=this.plan[this.i]){
    const ex=EXERCISES[id];
    return item?.targetOverride ?? item?.repsOverride ?? item?.durationOverride ?? this.state.targets[id] ?? ex.base
  }
  ...
  enter(){
    ...
    if(ex.mode==="time" || item.durationOverride) this.startTimer(item,ex,target);
  ...
  record(early=false){
    ...
    this.log.push({id:item.id,name:ex.name,mode:ex.mode,target,actual,effort:this.effort,skipped:false,early});
  }
  skip(){
    const item=this.plan[this.i];if(item?.kind==="exercise"&&!item.warm){const ex=EXERCISES[item.id];this.log.push({id:item.id,name:ex.name,mode:ex.mode,target:this.target(item.id,item),actual:null,effort:"skipped",skipped:true})}
  ...
  pause(){const ex=this.ex();if(!ex||ex.mode!=="time")return;if(this.running){this.stop();this.hooks.paused?.(true)}else{this.startTimer(this.plan[this.i],ex,this.remaining);this.hooks.paused?.(false)}}
  adapt(){
    const grouped={};
